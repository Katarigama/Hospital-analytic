1)  Из-за того, что нет доступа к clickhouse на пк выполнил на postgresql, в паре мест синтаксис может отличаться, но не слишком критично. Для выполнения тз я развернул на локалке сервер с тестовыми данными.
2) Вначале я сделал неоптимизированный запрос, выполняющий задание:
